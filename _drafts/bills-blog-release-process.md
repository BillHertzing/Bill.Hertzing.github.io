...
The criteria and steps to create new branches is discussed in [Bill's Blog New Branch criteria](TBD). This document assumes that there are "more than one" active branches, and that all development work is being tracked in a branch. This document is focused on the steps needed to release one active development branch into `main`, and update the production site with the work done on the development branch.
...
1. If there are any recent updates to `main`, then rebase the Release Candidate branch onto `main`, and recheck the Release Candidate branch with [Bill's Blog Pre-Release Checklist](bills-blog-pre-release-checklist.md) to ensure the changes didn't break anything, and restart this release process.
